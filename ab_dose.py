...
def convolution(tia_map: sitk.Image, kernel: np.array, path:str):

    """
    performs the convolution of the TIA map and the S-value kernel, via FFT convolution.
    ...
    np_tia_map = sitk.GetArrayFromImage(tia_map)*(image_spacing[0]*image_spacing[1]*image_spacing[2]*(1e-3))/6783 # normalization to the administred activity
...
def density_correction(ct_path:str, output_density_path:str, abdose_path:str):

    abdose_array= sitk.GetArrayFromImage(functions.read_dicom(abdose_path))    

    ct_image= functions.read_dicom(ct_path)
    ct_array= sitk.GetArrayFromImage(ct_image)

    #ct-calibration to density
    threshold = 127.34355
    ct_density = np.where(ct_array < threshold,
        (0.00102430 * ct_array + 1.02419109),  
        (0.000503556 * ct_array + 1.090504592))
    
    ct_density_img= sitk.GetImageFromArray(ct_density)
    ct_density_img.SetSpacing(ct_image.GetSpacing())
    ct_density_img.SetOrigin(ct_image.GetOrigin())
    ct_density_img.SetDirection(ct_image.GetDirection())
    sitk.WriteImage(ct_density_img, output_density_path)

    resampled_ct= read_resample(output_density_path, abdose_path)

    print(resampled_ct.shape)
    corrected_map= abdose_array*(1.02/resampled_ct)
